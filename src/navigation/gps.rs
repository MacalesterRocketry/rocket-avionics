...
pub struct GpsState {
    #[serde(serialize_with = "serialize_instant_opt")]
    #[serde(deserialize_with = "deserialize_instant_opt")]
    pub last_packet_time: Option<Instant>, // TODO: We should probably combine Instant and DateTime one shared struct that implements Serialize.
    #[serde(serialize_with = "serialize_instant_opt")]
    #[serde(deserialize_with = "deserialize_instant_opt")]
...
impl GpsState {
    ...
    fn process_packets(&mut self, it: &mut UbxParserIter<FixedBuffer<1024>>) {
        loop {
            match it.next() {
    ...
    fn process_valid_packet(&mut self, p: PacketRef) {
        match p {
...
pub async fn gps_loop(gps_config: GpsConfig) {
    let mut uart = init_gps(gps_config).await;
...
async fn init_gps(gps_config: GpsConfig) -> BufferedUart {
    let c = gps_config;
